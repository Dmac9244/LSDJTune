...
# JHL (abrasive) 2009 (Updated by Counter-chip & Johan 2020)
# tested with LSDJ 8.7.1

#use Data::Dumper;
use File::Copy;

print "lsdj_tune 1.5 -- for LSDJ 8.7.1 - abrasive 24/3/2009 (Updated 6/25/2020)\n";

sub freq2reg {
    my ($freq, $reg);
    my @regs;
    while ($freq = shift) {
        $reg = int(2048.5-131072/$freq);
        if ($reg <= 0) {
            $reg = 1;
...
sub find_tables {
    ...
    for (my $offset = 0; $offset < 0x8000; $offset++) {
        if (!$tune_loc) {
